...
- ❌ **不修改测试以适配错误的实现**：测试跑不过时，唯一允许的方向是**修实现让测试过**；不允许把 `expect(x).toBe(3)` 改成 `expect(x).toBeDefined()` 这类弱化断言来骗绿。如果测试本身写错了（少见），必须在 commit message 里显式说明"修改测试因为原断言错误：原期望 X，实际正确行为 Y"，并附最小复现的人工解释。
- ❌ **不删测试以解决失败**：测试失败不能通过 `delete it.skip(...)` / 注释掉来绕过。只能修实现、或修测试+解释（见上一条）、或确认是 flaky 单独标记 `it.skip` + 留 issue 链接。
- ❌ **不删功能以解决报错**：实现报错不能通过删调用点来"修"。先 understand 再修。
